...
const EmployeeManagementPage = () => {
  ...
  const handleDelete = async (userId: string) => {
    ...
    try {
      ...
      if (profileError) {
        ...
        throw new Error(
          `Tidak dapat memverifikasi status admin: ${profileError.message}`,
        );
      }

      if (currentProfile?.role !== "admin") {
        throw new Error(
          "Anda tidak memiliki izin untuk menghapus pegawai. Role saat ini: " +
            (currentProfile?.role || "tidak ditemukan"),
        );
      }
      ...
      toast({
        title: "Berhasil",
        description: `Pegawai ${targetEmployee?.name || "Unknown"} berhasil dihapus`,
      });
  ...
  const filteredEmployees = employees.filter(
    (emp) =>
      ...
      (emp.nip && emp.nip.toLowerCase().includes(searchTerm.toLowerCase())) ||
        ...
        <CardContent>
          <div className="flex gap-4 mb-6">
            <div className="relative flex-1">
              ...
              <Input
                placeholder="Cari pegawai berdasarkan nama, ID, NIP, atau departemen..."
                value={searchTerm}
